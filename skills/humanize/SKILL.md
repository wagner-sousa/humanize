...
**1. Pattern Breaker.** Read the text looking specifically for the structural signature of AI writing: identical paragraph lengths, a bullet list where prose would read more naturally, a topic sentence that restates the heading, rule-of-three lists ("robust, scalable, and maintainable"), transitions like "Additionally" / "Furthermore" / "It is worth noting" doing no real work, throat-clearing openers ("Great question!", "Certainly, I would be happy to..."), unearned closing summaries. Break the symmetry — vary sentence length, cut a transition that is not earning its place, let one paragraph run longer than its neighbors if that is how the content actually breaks. For short texts (a two- or three-sentence PR description, a single-paragraph comment) the symmetry problem is inside the paragraph, not between paragraphs — watch for every sentence landing at roughly the same length and clause count, and break that rhythm even when there is only one paragraph to work with.
...
**4. Imperfection Injector.** Perfect text has no story behind it. Where it fits naturally, add the kind of texture someone who actually lived the problem would include — an aside that shows judgment, a sentence that trails into the next thought instead of closing cleanly, an observation stated plainly instead of hedged. This is about *phrasing* texture, not new content: never invent a reason, a prior problem, or a piece of backstory the source did not state, even a plausible-sounding one — that is a fact violation, not a style choice. Do not force this into every paragraph, and do not "fix" it afterward — a slightly rough edge that came from a real observation is the point. Check your own output after this pass: if every sentence is still purely factual with no trace of judgment or stance anywhere, the pass did not actually happen, it just got acknowledged and skipped — go back and find at least one place, however small, where the source supports a point of view (this was the right call, this part was messy, this is what mattered) and let it show.

**5. Audio Test.** Read the current draft as if speaking it aloud. Mark every sentence where you would stumble, pause oddly, or where the rhythm breaks from how a person actually talks. Watch specifically for em-dash strings doing the job of commas and periods, and for long compound sentences where multiple clauses are chained by comma + "so" or comma + "and" — that construction sounds calibrated, not spoken. Rewrite those specific sentences so they flow as speech — this pass is surgical, only touching the sentences that failed the read-aloud check, not a rewrite of the whole thing again.
